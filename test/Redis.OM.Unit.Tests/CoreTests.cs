...
namespace Redis.OM.Unit.Tests
{
	public class CoreTests
	{
		...
		[Fact]
		public void SimpleGetWithQueryFilter()
		{
			var host = Environment.GetEnvironmentVariable("STANDALONE_HOST_PORT") ?? "localhost";
			var provider = new RedisConnectionProvider($"redis://{host}");
			var connection = provider.Connection;
			var collection = provider.RedisCollectionWithQueryFilter<HashPerson>(x => x.Name == "isn't Steve" && x.Age == 33);

			connection.DropIndex(typeof(HashPerson));
			connection.CreateIndex(typeof(HashPerson));

			var obj = new HashPerson() { Name = "Steve", Age = 33, Email = "foo@bar.com" };
			var key = collection.Insert(obj);

			var people = collection.Where(x => x.Name == "Steve").Where(x=>x.Age == 33).ToList();

			Assert.Empty(people);
		}

		[Fact]
		public void SimpleGetWithIgnoreQueryFilter()
		{
			var host = Environment.GetEnvironmentVariable("STANDALONE_HOST_PORT") ?? "localhost";
			var provider = new RedisConnectionProvider($"redis://{host}");
			var connection = provider.Connection;
			...
			var people = collection
				.IgnoreQueryFilter()
				.Where(x => x.Name == "Steve").Where(x => x.Age == 33).ToList();
